...
class CLVernierCaliperView: UIView {
    private var configure: CLVernierCaliperConfigure = .init()
    private var section: Int = 0
    private var limitDecimal: NSDecimalNumber = .init(value: 0)
    ...
    var indexValueCallback: ((String) -> Void)?
...
extension CLVernierCaliperView {
    private func setRealValue(realValue: CGFloat, animated: Bool) {
        collectionView.setContentOffset(CGPoint(x: round(realValue) * CGFloat(configure.gap), y: 0), animated: animated)
    }
}
...
extension CLVernierCaliperView {
    /// 更新配置
    func updateConfigure(_ configureCallback: (CLVernierCaliperConfigure) -> Void) {
...
extension CLVernierCaliperView: UICollectionViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        let value = Int(scrollView.contentOffset.x / CGFloat(configure.gap))
        var realValue = CGFloat(value) * configure.minimumUnit + configure.minValue
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        if !decelerate {
            setRealValue(realValue: (scrollView.contentOffset.x) / CGFloat(configure.gap), animated: true)
        }
    }
...
extension CLVernierCaliperView: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if indexPath.item == 0 || indexPath.item == section + 1 {
            return CGSize(width: frame.width * 0.5, height: collectionView.frame.height)
        }
