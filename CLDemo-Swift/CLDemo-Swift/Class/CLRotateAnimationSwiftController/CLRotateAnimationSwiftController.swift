...
class CLRotateAnimationSwiftController: CLController {
    ...
    override func viewDidLoad() {
        ...
        do {
            let rotateAnimationView: CLRotateAnimationView = .init(frame: CGRect(x: 80, y: 120, width: 80, height: 80))
            rotateAnimationView.updateWithConfigure { configure in
            ...
            view.addSubview(rotateAnimationView)
        }

        do {
            let roundAnimationView = CLRoundAnimationView(frame: CGRect(x: 90, y: 250, width: 90, height: 90))
            roundAnimationView.updateWithConfigure { configure in
