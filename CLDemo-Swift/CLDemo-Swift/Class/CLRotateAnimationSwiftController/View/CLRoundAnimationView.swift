...
class CLRoundAnimationViewConfigure: NSObject {
    /// 外圆背景色
    var outBackgroundColor: UIColor = .lightGray
    /// 内圆背景色
    var inBackgroundColor: UIColor = .orange
    /// 外圆线宽
...
class CLRoundAnimationView: UIView {
    private let backgroundLayer: CALayer = .init()
    private let animationLayer: CALayer = .init()
    private let rotationAnimation: CABasicAnimation = .init(keyPath: "transform.rotation.z")
    private let configure: CLRoundAnimationViewConfigure = .defaultConfigure()
    private var width: CGFloat = 0.0
    private var height: CGFloat = 0.0
    private var isPause: Bool = false

    /// 更新配置
    func updateWithConfigure(_ configureBlock: ((CLRoundAnimationViewConfigure) -> Void)?) {
