...
class CLBubblePopupViewConfigure {
    /// 箭头大小
    var arrowSize: CGSize = .init(width: 10, height: 10)
    /// 边框圆角
...
class CLBubblePopupView: UIView {
    ...
    private var configure: CLBubblePopupViewConfigure = .init()
    private var blackOverlay: UIControl = .init()
    private var containerView: UIView!
    private var contentView: UIView!
    private var contentViewFrame: CGRect!
    private var arrowShowPoint: CGPoint!

    init(configureCallback: ((CLBubblePopupViewConfigure) -> Void)? = nil) {
...
extension CLBubblePopupView {
    ...
    private func create() {
        ...
        if configure.showShadowy {
            ...
            layer.shadowColor = UIColor(red: 32.0 / 256.0, green: 32.0 / 256.0, blue: 32.0 / 256.0, alpha: 0.5).cgColor
