...
class CLChatRecordView: UIView {
    ...
    /// 结束录音
    var finishRecorderCallBack: ((TimeInterval, Data) -> Void)?
    /// 是否正在录制
    ...
    private lazy var waveView: LottieAnimationView = {
        let view = LottieAnimationView(name: "recoredWave_dk")
        view.loopMode = .loop
        view.isHidden = true
    ...
    private lazy var iconImageView: UIImageView = {
        let iconImageView = UIImageView(image: UIImage(named: "icon_voice"))
    ...
    private func endLongPress() {
        if isRecording {
            ...
            if isOut {
                ...
                cancelRecord()
                if CLPermissionsManager.isAllowed(.microphone) {
                    CLPopoverManager.showTips(text: "录音时间过短")
                }
            } else {
                endRecord()
            }
...
extension CLChatRecordView {
    private func startRecord() {
        if CLPermissionsManager.isAllowed(.microphone) {
            ...
            CLPermissionsManager.request(.microphone) { status in
