//
//  CLMailboxAutoCompletionViewController.m
//  CLDemo
//
...
@interface CLMailboxAutoCompletionViewController ()<CLMailBoxTextViewDelegate>
...
@implementation CLMailboxAutoCompletionViewController
...
- (void)textViewEndEditing:(CLMailBoxTextView *)texteView {
    CLLog(@"结束输入邮箱---->%@",texteView.text);
}
///输入改变
- (void)textViewDidChange:(CLMailBoxTextView *)texteView {
    CLLog(@"当前邮箱输入----->%@",texteView.text);
}
