...
class CLTagsFrameConfigure: NSObject {
    ...
    /// 标签内部字体大小
    var tagsTitleFont: UIFont = UIFont.systemFont(ofSize: 15)
    /// 是否对齐
...
class CLTagsFrameHelper: NSObject {
    
}
extension CLTagsFrameHelper {
    ...
    class func calculateTagsFrame(configure: ((CLTagsFrameConfigure) -> ()), tagsArray: [String]) -> (tagsFrames: [CGRect], tagsHeight: CGFloat) {
        ...
        for i in 0..<tagsArray.count {
            textWidht = (tagsArray[i] as NSString).size(withAttributes: attributedFont).width + defaultConfigure.tagsMinPadding * 2
